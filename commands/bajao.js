...
module.exports = {
    ...
    async execute(message, args){
        ...
        if(!voiceChannel) return message.channel.send('Voice channel pe ayo betaji :slightly_smiling_face:');
            const permissions = voiceChannel.permissionsFor(message.client.user);
            if(!permissions.has('CONNECT')) return message.channel.send('Permission mangta hain!');
            if(!permissions.has('SPEAK')) return message.channel.send('Permission mangta hain!');
